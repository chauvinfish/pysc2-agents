...
flags.DEFINE_enum("difficulty",'very_hard', ['very_easy','easy','medium','medium_hard','hard','very_hard','cheat_vision','cheat_money','cheat_insane']
, "Bot's strength.")
flags.DEFINE_integer("batch_steps",60, "A batch's steps.")
flags.DEFINE_integer("max_agent_steps",2048, "Total agent steps.")
    ...
    for recorder, is_done in run_loop([agent], env, MAX_AGENT_STEPS):
      if FLAGS.training:
        ...
        if is_done:
          ...
          for i in range(int(len(replay_buffer)/BATCH_STEPS)):

            agent.update(replay_buffer[i*BATCH_STEPS:(i+1)*BATCH_STEPS], FLAGS.discount, learning_rate, counter)
          replay_buffer = []
